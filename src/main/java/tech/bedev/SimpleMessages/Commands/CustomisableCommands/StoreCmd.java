...
public class StoreCmd implements Listener, CommandExecutor {
        ...
        if (sender instanceof Player) {
            if (cmd.getName().equalsIgnoreCase("store")) {
                if (plugin.getConfig().getBoolean("Store.Enabled")) {
                    if (!sender.hasPermission("sm.store")) {
                        sender.sendMessage(ChatColor.RED + "You do not have permission to use this command!");
                    } else {
                        String cmdText = ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString("Store.Message"));
